...
# Importing Data
df = pd.read_csv("product_sentiment.csv", encoding='ISO-8859–1',dtype='str')
...
database_name = 'HDIdatabase'
database = client.get_database_client(database_name)
container_name = 'ProductSentiment'

try:
    container = database.create_container(id=container_name, partition_key=PartitionKey(path="/product_id"))
except exceptions.CosmosResourceExistsError:
